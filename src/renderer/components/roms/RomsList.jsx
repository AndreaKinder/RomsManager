    ...
    <div className="roms-grid">
      {roms.map((rom) => (
        <div key={rom.id} className="nes-container is-dark rom-card">
          <CoverImage
            coverPath={rom.cover_path}
            customCoverPath={rom.custom_cover_path}
            title={rom.metadata.title || rom.file_name}
          />
          ...
          <div className="rom-info">
            <p>
              <strong>Console:</strong> {rom.console || "Unknown"}
            </p>
            <p>
              <strong>File:</strong> {rom.file_name}
            </p>
            {rom.metadata.genre && (
              <p>
                <strong>Genre:</strong> {rom.metadata.genre}
              </p>
            )}
            {rom.metadata.year && (
              <p>
                <strong>Year:</strong> {rom.metadata.year}
              </p>
            )}
          </div>
          ...
          <div className="rom-actions">
            ...
            <button
              ...
              onClick={() => {
                if (
                  confirm(`Delete "${rom.metadata.title || rom.file_name}"?`)
                ) {
                  onDelete(rom.id);
                }
              }}
