...
describe("FilePickerInput", () => {
  test("renders with label and readonly input", () => {
    render(<FilePickerInput label="ROM File" value="" onFilePick={() => {}} />);

    expect(screen.getByText("ROM File")).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Select a ROM file..."),
    ).toBeInTheDocument();
  });
  ...
  test("calls onFilePick when Browse button is clicked", () => {
    ...
    render(
      <FilePickerInput label="ROM File" value="" onFilePick={handleFilePick} />,
    );
